...
mysql cpu 25, ворсинки не растут.
...
mysql cpu 280-300, gunicorn 130. Уперлись в потолок cpu.

1.12. lfcb + fcb + cb + b + lcb + lb + fb, rps=1000
```
...
Проверим, поможет ли добавление других индексов для "полного покрытия" селекта (lcb, lb, fb).
...
mysql предпочетает делать index range scan по lb, видимо с учетом высокой селективности last_name.
...
из-за низкой этих подключей в других индексах (где работает Index lookup with condition).
...
2.5. Попробуем write без индексов. rps=200 
```
...
### Выводы
Индексы необходимы для многих жизненно важных селектов, но они сильно замедляют запись.
Индексы должны добавляться только если это действительно необходимо и подтверждено тестами.
